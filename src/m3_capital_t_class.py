...
def horizontal(point, width, height):
    point1 = rg.Point(point.x + width / 2, point.y + height / 2)
    point2 = rg.Point(point.x - width / 2, point.y - height / 2)
    return rg.Rectangle(point1, point2)

def vertical(point, width, height):
    point1 = rg.Point(point.x + width / 2, point.y + height - width / 2)
    point2 = rg.Point(point.x - width / 2, point.y - width / 2)
    return rg.Rectangle(point1, point2)
    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   dimensions.pdf   to understand the following! ***
        ...
        # ---------------------------------------------------------------------
        self.h_rect = horizontal(intersection_center, width, letter_thickness)
        self.v_rect = vertical(intersection_center, letter_thickness, height)
    ...
    def clone(self):
        ...
        wide = self.h_rect.get_width()
        high = self.v_rect.get_height()
        thick = self.v_rect.get_width()
        newT = CapitalT(center, wide, high, thick)
